...
/**
 * Handles image upload by sending the file to the API route,
 * then inserts the image into the Tiptap editor.
 * @param editor The Tiptap editor instance.
 * @param file The file to upload.
 * @param onProgress Optional callback for tracking upload progress (not fully implemented for fetch API).
...
export const handleImageUpload = async (
  ...
  if (file.size > MAX_FILE_SIZE) {
    throw new Error(
      `La taille du fichier (${(file.size / (1024 * 1024)).toFixed(
        2
      )}MB) dépasse le maximum autorisé (${MAX_FILE_SIZE / (1024 * 1024)}MB).`
    );
  }
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(errorData.error || "Failed to upload image to API");
    }
...
/**
 * Converts a File to base64 string (peut être supprimé si vous n'utilisez plus le base64)
